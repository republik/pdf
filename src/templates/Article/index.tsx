...
const legend = {
  matchMdast: matchParagraph,
  component: Legend,
  rules: [legendEmphasis, ...interactionParagraphRules],
};
...
const getImageProps = (node, index, parent, { ancestors }) => {
  ...
  return {
    ...
    isCover: ancestors.length === 2 && rootNode.children.indexOf(parent) === 0,
  };
};
...
const figure = {
  ...
  props: (node, index, parent, { ancestors }) => {
    ...
    return {
      ...
      inCenter,
      aspectRatio,
      size: rootNode.options.images ? node.data.size : undefined,
